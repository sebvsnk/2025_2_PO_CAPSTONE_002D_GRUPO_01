...
.table-scroll {
  max-height: 65vh; /* Limita la altura al 65% de la pantalla. Ajústalo si prefieres px (ej. 600px) */
  overflow-y: auto; /* Activa el scroll vertical cuando se pasa de altura */
  overflow-x: auto;
}
...
/* Nota: El estilo visual del scroll (colores, grosor) lo heredará automáticamente de main.css */
...
.scroll-aurora::-webkit-scrollbar-corner {
  background: rgba(15, 23, 42, 0); /* Transparente */
}

.audit-table {
  width: 100%;
  border-collapse: separate; /* Cambiado de 'collapse' a 'separate' para que funcione mejor el border-radius y el sticky */
  border-spacing: 0; /* Elimina espacios entre celdas */
  min-width: 900px;
}

/* === NUEVO: Encabezado Fijo (Sticky Header) === */
/* Esto hace que los títulos de la tabla se queden fijos al hacer scroll */
.audit-table th {
  position: sticky;
  ...
  background: rgba(
    ...
    46,
    0.95
  ); /* Fondo casi opaco para que no se trasluzca el texto de abajo al scrollear */
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.2); /* Sombra sutil para separarlo del cuerpo */
}
.audit-table td {
  padding: 16px 20px;
