...
from .models import (
    Vehiculo, BitacoraPorteria, Ot, Usuario, Estado, Tarea, Pausa, Evidencia, AuditLog, Repuesto, TareaRepuesto, VehiculoPatenteHist
)
...
from .serializers import (
    ...
    AuditLogSerializer, TareaRepuestoSerializer, TareaRepuestoCreateSerializer, OtExportSerializer,TareaExportSerializer, OtExportSerializer, OtDetailSerializer, VehiculoUpdateSerializer,  # ¡El que faltaba!
)
...
class VehiculoList(generics.ListAPIView):
    ...
    # Versión estable: Trae todos los vehículos ordenados por fecha
    queryset = Vehiculo.objects.all().order_by("-creado_en")
    
    # Usamos el serializer original que ya tenías funcionando
...
class OtDetailUpdateView(mixins.RetrieveModelMixin,
    ...
    """(GET) Detalle OT con tareas completas. (PUT/PATCH) Actualiza descripción."""
    
    # CORRECCIÓN: Agregamos prefetch_related para traer las tareas y sus estados
    queryset = Ot.objects.select_related(
        "vehiculo", "estado", "creado_por"
    ).prefetch_related(
        "tareas",               # <--- ESTO FALTABA: Trae la lista de tareas
        "tareas__estado",       # <--- Trae el estado de cada tarea
        "tareas__responsable"   # <--- Trae el mecánico de cada tarea
    ).all()
    ...
    def get_serializer_class(self):
        ...
        # Usamos el serializer detallado para ver la lista de tareas
        ...
        if original_descripcion != instance.descripcion:
             ...
             register_audit_log(
                ...
                details={
                    ...
                    "by_partial_update": partial
                }
            )

        # Devolvemos el serializer detallado para que la UI se actualice correctamente
        read_serializer = OtDetailSerializer(instance, context=self.get_serializer_context())
...
# (También asegúrate de que 'serializers' de rest_framework esté importado)
from rest_framework import serializers

# ... (tus otras vistas) ...

class PublicVehicleStatusView(generics.RetrieveAPIView):
    """
    Vista PÚBLICA (sin autenticación) para que un cliente externo
    consulte el estado de la OT de su vehículo.
    Busca por patente.
    """
    permission_classes = [permissions.AllowAny]
    
    # Usamos el serializer que acabamos de arreglar
    serializer_class = PublicOtStatusSerializer 
    
    # El queryset debe incluir los estados que el público puede ver
    queryset = Ot.objects.filter(
        # Mostramos activas, pausadas y CERRADAS
        estado__code__in=['ACTIVA', 'PAUSADA', 'CERRADA'] 
    ).select_related('vehiculo', 'estado').prefetch_related('tareas', 'tareas__estado')
    
    # El 'lookup_field' de la URL de urls.py es 'patente'
    lookup_field = 'patente' # <-- Esto debe coincidir con tu urls.py

    def get_object(self):
        # --- ¡ESTE ERA EL BUG 404! ---
        
        # 1. Obtenemos la patente de la URL (ej: "CJHD92")
        patente_kwarg = self.kwargs.get(self.lookup_field) # Usa el lookup_field
        
        if not patente_kwarg:
            raise Http404("Patente no proporcionada.")

        # 2. Buscamos el vehículo
        try:
            vehiculo = Vehiculo.objects.get(patente=patente_kwarg.upper())
        except Vehiculo.DoesNotExist:
            # Usamos el mismo formato de error que el frontend espera
            raise serializers.ValidationError({"detail": "Vehículo no encontrado."})
        
        # 3. Buscamos la OT más reciente (activa, pausada o cerrada) para ese vehículo
        # Usamos .first() porque order_by '-fecha_apertura' ya está en el Meta del modelo OT
        ot = self.get_queryset().filter(vehiculo=vehiculo).first() 
        
        if not ot:
            # Usamos el mismo formato de error que el frontend espera
            raise serializers.ValidationError({"detail": "No hay OT activa o reciente para este vehículo."})
...
class TableroView(APIView):
    """
    (GET) Datos para el Tablero Kanban (RF-TAB-01).
    Optimizado: Excluye choferes para carga instantánea.
    ...
    def get(self, request):
        # --- ⚡ CONSULTA OPTIMIZADA ⚡ ---
        # 1. Trae OTs activas (excluye cerradas/anuladas).
        # 2. Usa select_related SOLO para lo visual: Vehículo, Estado, Creador.
        # 3. NO trae 'vehiculo__chofer', cortando el cuello de botella.
        ...
        if estados_filter:
            codigos = [e.strip().upper() for e in estados_filter.split(',')]
            queryset = queryset.filter(estado__code__in=codigos)

        patente_filter = request.query_params.get('patente')
        if patente_filter:
            # Búsqueda de patente sin guiones
            patente_limpia = patente_filter.upper().replace('-', '')
            queryset = queryset.filter(vehiculo__patente__icontains=patente_limpia)
        ...
        try:
            ...
            if fecha_inicio:
                dt_inicio = timezone.make_aware(datetime.fromisoformat(fecha_inicio).replace(hour=0, minute=0, second=0), tz)
                queryset = queryset.filter(fecha_apertura__gte=dt_inicio)
            
            if fecha_fin:
                dt_fin = timezone.make_aware(datetime.fromisoformat(fecha_fin).replace(hour=23, minute=59, second=59), tz)
                queryset = queryset.filter(fecha_apertura__lte=dt_fin)
        except (ValueError, TypeError):
             return Response({"detail": "Fecha inválida."}, status=status.HTTP_400_BAD_REQUEST)

        responsable_id = request.query_params.get('responsable_ot_id')
        if responsable_id:
            queryset = queryset.filter(creado_por_id=responsable_id)

        # --- AGRUPACIÓN ---

        ots_agrupadas = defaultdict(list)
        
        # Obtener estados para asegurar columnas vacías si es necesario
        estados = Estado.objects.filter(tipo='ot', activo=True).exclude(code__in=['CERRADA', 'ANULADA']).order_by('orden')
        
        for estado in estados:
            ots_agrupadas[estado.code] = []

        # Serializar
        # IMPORTANTE: TableroOtSerializer debe usar VehiculoTableroSerializer (sin chofer)
        serializer = TableroOtSerializer(queryset, many=True)
        
        for ot_data in serializer.data:
            # Usamos .get() para evitar errores si el estado no tiene 'code' por alguna razón
            estado_data = ot_data.get('estado')
            if estado_data:
                code = estado_data.get('code')
                if code in ots_agrupadas:
                    ots_agrupadas[code].append(ot_data)
...
class AdminUsuarioListCreateView(generics.ListCreateAPIView):
    ...
    Requiere rol ADMIN.
    """
    queryset = Usuario.objects.prefetch_related('vehiculos_asignados').all().order_by('nombre')
    ...
    def create(self, request, *args, **kwargs):
        ...
        # --- 🛑 INICIO SOLUCIÓN: VALIDACIÓN PREVIA LOCAL ---
        # Verificamos si ya existe un usuario con ese email en NUESTRA base de datos (Django)
        # Usamos iexact para que no importe mayúsculas/minúsculas
        if Usuario.objects.filter(email__iexact=email).exists():
            return Response(
                {"detail": "Error: Ya existe un usuario registrado con este correo en el sistema."},
                status=status.HTTP_400_BAD_REQUEST
            )
        
        # Verificamos si ya existe el RUT (si se proporcionó) en NUESTRA base de datos
        if rut and Usuario.objects.filter(rut=rut).exists():
            return Response(
                {"detail": "Error: Ya existe un usuario registrado con este RUT."},
                status=status.HTTP_400_BAD_REQUEST
            )
        # --- 🏁 FIN SOLUCIÓN ---
        ...
        try:
            ...
            response_supabase = supabase_admin.auth.admin.invite_user_by_email(
                email, options={'redirectTo': REDIRECT_URL} 
            )
            supabase_user_id = response_supabase.user.id 
        except Exception as e:
            # Manejo de errores de Supabase
            if 'already been registered' in str(e):
                return Response({"detail": "Un usuario con este correo ya existe (Supabase Auth)."}, status=status.HTTP_400_BAD_REQUEST)
            return Response({"detail": f"Error Supabase: {e}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

        # 3. Crear Perfil Local
        try:
            nuevo_perfil = Usuario.objects.create(
                nombre=nombre, email=email, rol=rol,
                rut=rut, numero_telefonico=numero_telefonico, 
                external_id=supabase_user_id
            )
            
            # --- Auditoría ---
            try:
                # Obtenemos el ID del admin que está haciendo la acción
                admin_user = Usuario.objects.get(external_id=request.user.username)
            ...
            register_audit_log(
                user_id=admin_id,
                action_type="ADMIN_USER_INVITE",
                entity_type="usuario",
                entity_id=nuevo_perfil.id,
                details={
                    "email": email,
                    "rol": rol,
                    "nombre": nombre
            ...
            # Rollback en caso de error local (borramos el usuario de Supabase si falló la BD local)
            try: supabase_admin.auth.admin.delete_user(supabase_user_id) 
            except: pass
            return Response({"detail": f"Error local al guardar perfil: {e}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
# REEMPLAZA TU CLASE "ChoferContactoCreateView" POR ESTA:
class ChoferContactoCreateView(generics.CreateAPIView):
    """
    (POST) Crea o OBTIENE un perfil de Usuario con rol 'CHOFER' (Contacto de vehículo).
    Busca por RUT/Email. Si existe y es CHOFER, lo devuelve. Si no, lo crea.
    Si existe pero NO es CHOFER, devuelve un error.
    """
    permission_classes = [permissions.IsAuthenticated, IsSupervisorOrAdminUser] 
    queryset = Usuario.objects.all()
    ...
    def create(self, request, *args, **kwargs):
        ...
        rut_limpio = validated_data.get('rut')
        email = validated_data.get('email')

        # 2. Lógica "Get or Create" (Buscar por RUT o Email)
        existing_user = None
        if rut_limpio:
            # Buscamos si ya existe un usuario (chofer o no) con ese RUT
            existing_user = Usuario.objects.filter(rut=rut_limpio).first()
        
        if not existing_user and email:
            # Si no se encontró por RUT, intentar buscar por email (si no es nulo)
            existing_user = Usuario.objects.filter(email__iexact=email).first()

        if existing_user:
            # --- 🌟 INICIO DE LA CORRECCIÓN 🌟 ---
            
            # 2a. SI EXISTE, VERIFICAR EL ROL
            if existing_user.rol == 'CHOFER':
                # Es un Chofer existente, lo reutilizamos y actualizamos
                existing_user.nombre = validated_data.get('nombre', existing_user.nombre)
                existing_user.numero_telefonico = validated_data.get('numero_telefonico', existing_user.numero_telefonico)
                existing_user.email = validated_data.get('email', existing_user.email) # Actualizar email también
                existing_user.save()
                
                read_serializer = self.get_serializer(existing_user)
                # Devolvemos 200 OK. El frontend (VehicleCreateView)
                # leerá el ID y continuará sin problemas.
                return Response(read_serializer.data, status=status.HTTP_200_OK) 
            
            else:
                # 2b. SI EXISTE, PERO NO ES CHOFER (Ej: "PORTERIA")
                # Esto es un error. No podemos asignarle un vehículo.
                error_msg = (
                    f"El RUT ({rut_limpio}) ya está registrado y "
                    f"pertenece a un usuario con rol '{existing_user.rol}', no a un CHOFER."
                )
                # Devolvemos un 400 (Solicitud Incorrecta)
                return Response({"detail": error_msg}, status=status.HTTP_400_BAD_REQUEST)
            
            # --- 🌟 FIN DE LA CORRECCIÓN 🌟 ---

        # 3. SI NO EXISTE: Crear el nuevo usuario (esto no cambia)
        try:
            instance = serializer.save(
                rol='CHOFER', 
                external_id=None,
            ...
            # Manejar error de creación (ej. violación de constraint por si acaso)
            return Response({"detail": f"Error al crear el perfil: {e}"}, status=status.HTTP_400_BAD_REQUEST)
...
class OtHistorialListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        # (Tu función get_queryset se mantiene igual)
...
# ... (Asegúrate de tener: from .models import ..., VehiculoPatenteHist)

class VehiculoUpdateView(generics.UpdateAPIView):
    permission_classes = [permissions.IsAuthenticated, IsSupervisorOrAdminUser]
    queryset = Vehiculo.objects.all()
    
    # CAMBIO AQUÍ: Usamos el serializer de actualización
    serializer_class = VehiculoUpdateSerializer 
    
    lookup_url_kwarg = "vehiculo_id"

    def perform_update(self, serializer):
        # ... (El resto de tu lógica de historial se mantiene IGUAL) ...
        # Solo cambia la línea de serializer_class arriba.
        
        # COPIA TU LÓGICA DE HISTORIAL AQUÍ SI NO LA TIENES A MANO:
        vehiculo_actual = self.get_object()
        patente_vieja = vehiculo_actual.patente
        nueva_patente = serializer.validated_data.get('patente')

        if nueva_patente and nueva_patente != patente_vieja:
            # Importante: Asegúrate de haber importado VehiculoPatenteHist al inicio del archivo
            VehiculoPatenteHist.objects.create(
                vehiculo=vehiculo_actual,
                patente=patente_vieja,
                desde=timezone.now()
            )
            # (Lógica de auditoría opcional...)
